...
function createNewArray(noOfBars = 60) {
    ...
    // Determine the scaling factor based on screen width
    let scaleFactor;
    if (screenWidth <= 600) {
        // If screen width is less than or equal to 600px, use a scale factor of 1
        scaleFactor = 1;
    } 
    else if (screenWidth <= 780) {
        // Otherwise, use a scale factor of 2
        scaleFactor = 1.5;
    }
    else {
        // Otherwise, use a scale factor of 2
    ...
    for (let i = 0; i < noOfBars; i++) {
        ...
        // Adjust the height of the bars based on the scale factor
        bar.style.height = `${barArray[i] * scaleFactor}px`;
